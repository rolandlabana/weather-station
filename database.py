...
#RSL: added below
import pytz
#RSL: 7/29/22:  added changes to try to get the current date/time written into the records when
#stored. basically removed the evaluation of curr time at create time
...
#RSL: 7/29/22: edited the default for created to be None and then to grab curr time 
# compare with the orig in the BK version of this file

    def insert(self, ambient_temperature, ground_temperature, air_quality, air_pressure, humidity, wind_direction, wind_speed, wind_gust_speed, rainfall, created=None):
        if created is  None:
                tz_LA = pytz.timezone('America/Los_Angeles')
                created = datetime.datetime.now(tz_LA).strftime("%Y-%m-%d %H:%M:%S")
